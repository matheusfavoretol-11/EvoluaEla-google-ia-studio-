...
export default function CoachView() {
  ...
  return (
    <div className="p-6 space-y-10 bg-transparent min-h-full text-white font-sans relative overflow-hidden">
      {/* Background Glow */}
      ...
      <header className="relative z-10">
        ...
        <p className="text-sm font-bold text-white/40 uppercase tracking-widest">O corpo alcança o que a mente acredita.</p>
      </header>

      {/* Direct Support / Coach */}
      <section className="glass-morphism rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl border border-white/10">
        <div className="relative z-10">
          <h3 className="text-3xl font-bold mb-4 tracking-tight">Fale com seu <span className="text-[#C5A059]">Coach</span></h3>
          <p className="text-white/40 text-base mb-10 leading-relaxed font-bold">
        ...
        <div className="absolute inset-0 bg-black/40 backdrop-blur-xl flex flex-col items-center justify-center z-20 p-8 text-center border border-white/10 rounded-[2.5rem]">
          ...
          <button className="bg-gradient-to-r from-[#8B4357] to-[#C5A059] text-black font-bold py-4 px-10 rounded-2xl text-[10px] shadow-2xl hover:scale-105 transition-all uppercase tracking-widest">
            Conhecer os Planos
          </button>
        </div>
      </section>
      ...
      <section className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-[#C5A059]/90 flex items-center justify-center border border-white/10">
        ...
        <div className="grid gap-5">
          {articles.map(article => (
            <div key={article.id} className="glass-morphism p-6 rounded-[2rem] border border-white/10 flex justify-between items-center cursor-pointer hover:border-[#8B4357]/90 transition-all group shadow-2xl">
              <div className="flex-1">
                <h4 className="font-bold text-white mb-2 text-lg group-hover:text-[#8B4357] transition-colors tracking-tight">{article.title}</h4>
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest block">Leitura: {article.readTime}</span>
                  <div className="w-1 h-1 rounded-full bg-white/20"></div>
                  <span className="text-[10px] text-[#C5A059] font-bold uppercase tracking-widest block">Mindset</span>
                </div>
              </div>
              <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white/40 group-hover:bg-[#8B4357] group-hover:text-black transition-all shrink-0 border border-white/10">
