...
export default function CoachView() {
  ...
  return (
    <div className="p-6 space-y-10 bg-transparent min-h-full text-[var(--color-text)] font-sans relative overflow-hidden">
      {/* Background Glow */}
      ...
      <section className="glass-morphism rounded-[2.5rem] p-10 text-[var(--color-text)] relative overflow-hidden shadow-2xl border border-white/10">
        <div className="relative z-10">
          <h3 className="text-3xl font-bold mb-4 tracking-tight">Fale com seu <span className="text-[var(--color-accent)]">Coach</span></h3>
          <p className="text-[var(--color-text-muted)] text-base mb-10 leading-relaxed font-bold">
        ...
        <div className="absolute inset-0 bg-black/40 backdrop-blur-xl flex flex-col items-center justify-center z-20 p-8 text-center border border-white/10 rounded-[2.5rem]">
          <div className="w-20 h-20 rounded-3xl bg-[var(--color-accent)]/90 flex items-center justify-center mb-6 border border-white/10">
            <Lock className="text-black" size={32} />
          </div>
          <h4 className="font-bold text-3xl mb-3 tracking-tight">Acesso <span className="text-[var(--color-accent)]">Exclusivo</span></h4>
          <p className="text-sm text-[var(--color-text-muted)] mb-8 font-bold leading-relaxed">O chat direto com o coach está disponível apenas no Plano Premium.</p>
      ...
      <section className="relative z-10">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-[var(--color-accent)]/90 flex items-center justify-center border border-white/10">
        ...
        <div className="grid gap-5">
          {articles.map(article => (
            <div key={article.id} className="glass-morphism p-6 rounded-[2rem] border border-white/10 flex justify-between items-center cursor-pointer hover:border-[var(--color-primary)]/90 transition-all group shadow-2xl">
              ...
              <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-[var(--color-text-muted)] group-hover:bg-[var(--color-primary)] group-hover:text-black transition-all shrink-0 border border-white/10">
