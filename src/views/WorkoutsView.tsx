...
export default function WorkoutsView({ onUpgrade }: { onUpgrade: () => void }) {
  ...
  const handleCreateWorkout = () => {
    ...
    const newWorkout: Workout = {
      id: `my-${Date.now()}`,
      title: 'Novo Treino Personalizado',
      duration: '45 min',
      level: 'personalizado',
      calories: '350 kcal',
      premium: true,
      exercises: [
        { id: `ex-${Date.now()}-1`, name: 'Exercício 1', reps: '12 reps', sets: 3 },
        { id: `ex-${Date.now()}-2`, name: 'Exercício 2', reps: '12 reps', sets: 3 },
      ]
    };
                ...
                <>
                  ...
                  {myWorkouts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {myWorkouts.map(w => (
                        <WorkoutCard key={w.id} workout={w} onClick={handleWorkoutClick} hasAccess={hasAccess} />
                      ))}
                    </div>
                  )}
                </>
